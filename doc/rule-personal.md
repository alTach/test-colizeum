...
### 8 Работа с api
- все api методы и типы должны располагаться в отдельном файле и импортироваться через алиса (в будущем сетевой слой может быть заменён)
- для каждого api метода должен быть написан mapper который будет превращать серверные данные в клиентские, поэтому frontend не должне использовать серверные типы, если они совпадают а мы не хотим их дублировать, тогда просто создай свой алиас например PrizeFE = PrizeDto
- 
